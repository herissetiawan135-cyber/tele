...
export const run = {
  async: async (m, { conn, Api, body, Func, users, env, isROwner, isPrefix }) => {
    ...
    let json = JSON.parse(JSON.stringify(conn.tebakml[id]?.[1] || {}))
    
    if (!json.jawaban) return
    
    if (m.text == (isPrefix + "hml")) return conn.reply(m.chat, `<pre><code class="language-Clue">${json.jawaban.replace(/[AIUEOaiueo]/ig, '_')}</code></pre>`, m.msg, "HTML")

    if (!m.quoted || !m.text || !/Ketik.*hml/i.test(m.quoted.text) || /.*hgamb/i.test(m.text)) {
    ...
    if (m.quoted.id == conn.tebakml[id][0].message_id) {
        ...
        if (isSurrender) {
            ...
            clearTimeout(conn.tebakml[id][2])
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            await conn.reply(m.chat, caption, m.msg, "Markdown", [[{ text: 'Mainkan Lagi🎮', callback_data: '.tebakml' }]])
            clearTimeout(conn.tebakml[id][2])
            delete conn.tebakml[id]
